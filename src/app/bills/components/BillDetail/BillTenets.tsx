import { UnifiedBill } from "@/app/bills/utils/billConverters";
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {(bill.tenet_evaluations ?? []).map((tenet) => (
          <div key={tenet.id} className="border-l-4 border-border-light pl-4">
            <div className="flex items-start gap-3">
              <span
                className={`inline-flex items-center justify-center w-6 h-6 rounded-full border text-xs font-bold ${getAlignmentColor(tenet.alignment)}`}
              >
