          ...
          <section className="px-6 md:px-14 py-9 md:py-11 border-b-2 border-dark grid gap-9">
            ...
            <QuestionnaireRail headings={questionnaireHeadings(groups)}>
              <QuestionnaireCards
                ...
                respondents={mayoral}
                silent={[]}
                /* Only the sitting mayor gets a line under their name. The
                   other mayoral tags — "Declared", "Exploratory" — say where
                   a campaign is in its own life rather than what the reader
                   is choosing between, and printed under fourteen names on
                   thirty-three cards they would be noise. */
                roles={Object.fromEntries(
                  ballot
                    .filter((candidate) => candidate.tag === "Incumbent")
                    .map((candidate) => [candidate.key, "Incumbent"]),
                )}
                ballotSize={registered}
